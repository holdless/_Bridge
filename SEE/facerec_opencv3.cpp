...
//#include "opencv2/contrib/contrib.hpp"   //<-- not exists in opencv3.2.0
#include "opencv2/face.hpp" // opencv3 face rec
#include "opencv2/objdetect.hpp" //opencv3 face det
...
//String face_cascade_name = "D:/opencv/build/share/OpenCV/lbpcascades/lbpcascade_frontalface.xml";
String face_cascade_name = "D:/opencv/build/share/OpenCV/haarcascades/haarcascade_frontalface_alt.xml";
//String eyes_cascade_name = "D:/opencv/build/share/OpenCV/haarcascades/haarcascade_eye_tree_eyeglasses.xml";
...
/*  // 701.4.0105 hiroshi marks down: no use here
...
int main(int argc, const char *argv[])
{
	...
	argv[1] = "D:/Users/changyht/Documents/Visual Studio 2015/Projects/FaceRec/Debug/att_face_index.csv";
	//argv[2] = "output";
	argc = 2;
...
//	Ptr<FaceRecognizer> model; // <-- opencv 2
	Ptr<BasicFaceRecognizer> model; // <-- open 3
...
//	Mat W = model->getMat("eigenvectors"); // <-- opencv 2
	Mat W = model->getEigenVectors(); //< -- opencv 3
//	Mat mean = model->getMat("mean"); //<-- opencv 2
	Mat mean = model->getMean(); // <-- opencv 3
//	vector<Mat> projs = model->getMatVector("projections"); // <-- opencv2
	vector<Mat> projs = model->getProjections(); // <-- opencv 3
	Mat projections;
	start = clock();
	...
	*/
	...
	Mat testSample = imread("D:/Users/changyht/Pictures/face_database/jack/19131_1078086809441_8213360_n.jpg", CV_LOAD_IMAGE_COLOR);
...
//		Mat testFace_projection = subspaceProject(evs, mean, face_resized.reshape(1, 1));
		// opencv 3 new: Linear Discriminant Analysis cv::LDA
		Mat testFace_projection = LDA::subspaceProject(evs, mean, face_resized.reshape(1, 1));
		start = clock();
		...
		{
			float dis = norm(testFace_projection, projections.row(j), NORM_L2);
			float mdis = Mahalanobis(testFace_projection, projections.row(j), invcov);
			float sim = testFace_projection.dot(projections.row(j)) / (norm(testFace_projection)*norm(projections));
		...
		//string box_text = format("Prediction = %d", prediction); //<-- memory leak @ VS2015-opencv conflictio I guess
		//20161130 hiroshi: replaced by std::sprintf_s function
		char box_text[20];
		sprintf_s(box_text, 20, "Prediction = %d", prediction);
